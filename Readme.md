...
└── train/
    ...
    │   ├── 001.h5         # Contains long_img and trans_img datasets
    │   └── ...
    └── mask/                   # Annotation files (.h5 format)
        ├── 001_label.h5   # Contains long_mask, trans_mask, cls datasets
        └── ...
```
...
```bash
# Enter the docker.submission_template directory and build your Docker image
...
```bash
# Using the following code to login Docker
docker login

# "YOUR_USERNAME" remains your own Docker's username
# Tag your submission project
docker tag my-submission:latest YOUR_USERNAME/my-submission:latest
# Push it tu Docker Hub
docker push YOUR_USERNAME/my-submission:latest
```

 ⭐Then submit  `YOUR_USERNAME/my-submission:latest`  on the submission page.
...
# Upload my-submission.tar to your preferred cloud storage(Google Drive/Baidu Netdisk)
```

Then submit the download link on the submission page.

We provide three submission channels:

- Docker Hub (for Method A): Push the Docker image to Docker Hub and send the image link on the submission page.
